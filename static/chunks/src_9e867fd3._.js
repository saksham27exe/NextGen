(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/src_9e867fd3._.js", {
...
// import { db, auth as firebaseAuthInstance } from '@/config/firebase';
// Mock user data (replace with your actual mock data or logic)
const mockUser = {
    id: 'mockUser123',
    name: 'Test User',
...
function AuthProvider({ children }) {
    ...
    const [loading, setLoading] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(true); // Keep loading state for consistency
    // Simulate initial auth check (e.g., check localStorage or just set mock user)
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "AuthProvider.useEffect": ()=>{
            // Simulate checking auth status
            const storedUser = localStorage.getItem('mockUser'); // Example check
            if (storedUser) {
                try {
                    setAppUser(JSON.parse(storedUser));
                } catch (e) {
                    console.error("Failed to parse mock user from localStorage", e);
                    setAppUser(mockUser); // Fallback to default mock user
                    localStorage.setItem('mockUser', JSON.stringify(mockUser));
                }
            } else {
                // Set a default mock user if none stored, or leave null if login is required
                // setAppUser(mockUser); // Uncomment to auto-login mock user
                // localStorage.setItem('mockUser', JSON.stringify(mockUser)); // Store it
                setAppUser(null); // Default to logged out
            }
            setLoading(false);
        }
    }["AuthProvider.useEffect"], []);
    ...
    const login = (userData)=>{
        setAppUser(userData);
        localStorage.setItem('mockUser', JSON.stringify(userData)); // Example persistence
        setLoading(false);
    };
    ...
    const logout = ()=>{
        setLoading(true);
        setAppUser(null);
        localStorage.removeItem('mockUser'); // Clear mock user persistence
...
_s(AuthProvider, "2H467TkpSJ130k2CMg5CqIr9twI=");
_c = AuthProvider;
function useAuth() {
    _s1();
...
//# sourceMappingURL=src_9e867fd3._.js.map